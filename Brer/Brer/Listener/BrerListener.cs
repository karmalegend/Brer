    ...
    public sealed class BrerListener : IBrerListener
    {
        ...
        public IBrerListener StartListening()
        {
            ...
            foreach (var topic in Topics)
            {
                ...
                _channel.QueueBind(queue: _context.BrerOptions.QueueName, exchange: _context.BrerOptions.ExchangeName, routingKey: topic);
            }
        ...
        private void EventReceived(object? sender, BasicDeliverEventArgs e)
        {
            ...
            catch (Exception exception)
            {
                ...
                _context.Logger.LogError(exception,"Failed to handle event on exchange {ex} with routingkey {key}. Consumertag : {ct}",
                    e.Exchange, e.RoutingKey,e.ConsumerTag);
            }
        }
